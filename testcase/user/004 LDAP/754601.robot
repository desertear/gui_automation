*** Settings ***
Documentation    Verify that LDAP search window works correctly when mapping remote LDAP users to local users
...
@{expected_users}    test1    test11    test31
@{unexpected_users}    test1    test11    test3    test31    test4
&{data_dic}    ldap_server=${ldap_name}

*** Test Cases ***
754601
    #It's a duplicated case to 776623
    [Tags]    v6.2    chrome    754601    high
    ...
    Search user in ldap browser    Users    ${EMPTY}    1    ${True}    @{expected_users}
    Search user in ldap browser    Users    ${EMPTY}    2    ${False}    @{unexpected_users}
    Logout FortiGate
    Close Browser
